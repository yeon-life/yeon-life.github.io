...
> **갱신일**: 2026-05-27
> **상위 문서**: 연 통합 규격 **YUV6** (`연_통합규격_yuv4.md`)
> **버전 정책**: YLV 는 항상 YUV 와 같은 메이저 버전 (YUV6 → YLV-6, YUV5 → YLV-5)

**핵심 약속**: 이 문서는 **연라이프 미디어 사이트에 공통으로 적용되는 표준**만 담는다. YUV6 에 있는 공통 항목은 그대로 상속하고, 미디어 고유만 본 문서에 명시.
...
1. `y.life_배포규칙_YLV-6.md` (이 파일)
2. `y.life_배포규칙_YLV-6.html` (시각화)
...
- ★ 1-15. **비밀번호 평문 저장·조회 절대 금지** (YUV6 §3-4)
- ★ 1-16. **클라이언트에 비즈니스 로직·프롬프트 두지 않음** (YUV6 §19 Thin Client)
...
- ★ 1-18. **최종 배포/수정 후에는 사용자가 코드가 아닌 브라우저 렌더링 화면을 바로 확인할 수 있도록, 브라우저 스크린샷 이미지(캡처)를 답변 본문에 반드시 시각적으로 첨부(임베드)하여 보여줄 것** (YUV6 §1-18 상속)

---

## 2. 사용자 권한 5계층

YUV6 §2 (4계층 연아카데미형)을 연라이프 미디어형 **5계층**으로 확장.
...
### 2-2. 권한 누수 금지 (YUV6 §2-3 그대로)

---

## 3. y-프로젝트 SSO (YUV6 §3 상속 + 미디어 확장)
...
### 3-2. ⭐ 사용자 고유 번호 + ID/PW 변경 이력 (YUV6 §3-4 상속)
...
### 4-2. 5회 실패 5분 잠금 (YUV6 §4-2 그대로)

### 4-3. ⭐ 신규 가입자 본인 확인 (YUV6 §4-4 적용)
...
### 5-1. ⭐ 챗봇 기본 엔진 = Gemma 4 26B (YUV6 §5-1)
...
## 6. 자동 백엔드 탐색 (YUV6 §6 그대로 상속)

`https://api.y-life.kr` 활성화 시 자동 연결. `file://`·`localhost` 폴백.

---

## 7. 호스팅 마이그레이션 마커 (YUV6 §7 그대로 상속)
...
YUV6 §10 그대로 + 연라이프 신규 컬렉션 12개 (위 §3-4 참조).
...
04:00 · Gemini 15편 병렬 호출
05:00 · 본문 + 이미지 생성 완료
05:30 · 검수 큐 + 아침 알림
06:00 · 첫 발행
06:00~09:00 · 15편 순차 발행 (12분 간격)
```
...
| AI 기자 | 12 | 매일 1편 | Gemini 2.5 Flash |
...
- **최소화 광고 전략 (Minimal AdSense Strategy)**:
  - 텍스트 기반의 단순 캐릭터/문맥 매칭 광고를 기본으로 채택하여 독자 가독성을 해치지 않음.
  - 민감한 주제(정치, 선거, 성인 등) 및 이념 편향적 광고 카테고리는 애드센스 대시보드에서 전면 차단하여 제한적인 광고만 노출함.

---

## 18. ⭐ 자동 업데이트 시스템 (YUV6 §18 상속)
...
## 22. ⭐ Obsidian Vault 표준 (YUV6 §21 적용)
...
### 23-1. 파일명 규칙
`연라이프_<도메인>_<날짜>.<확장자>` (예: `연라이프_칼럼_2026-05-27.md`)
...
### Vault·내보내기·PWA (§22·§23·§24) ⭐ YUV6 신규
- [ ] Vault 4단 깊이?
- [ ] 6형식 내보내기 작동?
...
├── 세계화전략_지침.md / .html
├── 홍보전략_지침.md / .html
├── y.life_배포규칙_YLV-6.md / .html  ← 이 문서
...
본 규격서 변경 시:
1. `y.life_배포규칙_YLV-6.md`
2. `y.life_배포규칙_YLV-6.html`
3. 메모리 `project_yeon_life_homepage.md`
4. 본 문서 30번 "변경 이력"
5. §27 통합 업그레이드
...
| 버전 | 날짜 | 변경 사항 |
|---|---|---|
| **YLV-6** | 2026-05-27 | YUV6 동기화 승급. YUV6 신표준 5개 흡수: §18 자동 업데이트(Service Worker + idle 리로드), §21 배포·코드 보호(Cloudflare Pages + Workers + Firebase, Thin Client), §22 Obsidian Vault(4단·Wiki Link), §23 내보내기 v1.1.10(6형식), §24 PWA(manifest·SW·아이콘). §3-4 `y_user_no` + ID/PW 변경 이력 추가. §4-3 본인 확인 10일 유예 적용. §5-1 Gemma 4 26B 챗봇 기본 엔진. 비밀번호 평문 금지·Thin Client 명문화. 옛 YLV-3.2 `_백업/v_20260518_pre_YLV4/`로. |
| YLV-3.2 (백업) | 2026-05-27 | YUV-3.2 상속 + 미디어 고유 7개. 이름·버전 통일. "학원→연아카데미" 적용. |
